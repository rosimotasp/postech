...
def carregar_modelo(url):
    try:
        ...
        st.error("Erro ao carregar o modelo: arquivo inválido ou corrompido.")
        return None
...
if modelo is None:
    st.stop()  # Para o app se o modelo não carregar
...
altura = st.number_input("Altura (m)", 1.3, 2.2, 1.70, format="%.2f")
peso = st.number_input("Peso (kg)", 30.0, 200.0, 70.0, format="%.1f")
...
consumo_vegetais = st.slider("Frequência de consumo de vegetais (1 = baixa, 3 = alta)", 1, 3, 2)
refeicoes_dia = st.slider("Número de refeições principais por dia", 1, 4, 3)
lanches = st.selectbox("Costuma lanchar entre as refeições?", ["não", "às vezes", "frequentemente", "sempre"])
fuma = st.selectbox("Fuma?", ["sim", "não"])
consumo_agua = st.slider("Consumo de água (litros por dia)", 1, 3, 2)
controle_calorias = st.selectbox("Monitora o consumo de calorias?", ["sim", "não"])
atividade_fisica = st.slider("Atividade física por semana (horas)", 0, 4, 2)
uso_tecnologia = st.slider("Tempo usando dispositivos por dia (horas)", 0, 3, 1)
